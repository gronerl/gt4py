...
from gtc.common import CartesianOffset, DataType, ExprKind, LevelMarker
from gtc.oir import (
    AssignStmt,
    AxisBound,
    Expr,
...
class TestIntervalSet:
    @staticmethod
    def assert_consistency(iset: IntervalSet):
        assert len(iset.interval_starts) == len(iset.interval_ends)
        for i in range(len(iset.interval_starts) - 1):
            assert iset.interval_starts[i] < iset.interval_starts[i + 1]
            assert iset.interval_ends[i] < iset.interval_ends[i + 1]
            assert iset.interval_ends[i] < iset.interval_starts[i + 1]
        for start, end in zip(iset.interval_starts, iset.interval_ends):
            assert start < end
    ...
    def test_add(self, intervals, starts, ends):
        iset = IntervalSet()
        for interval in intervals:
            iset.add(interval)
            self.assert_consistency(iset)

        assert len(starts) == len(iset.interval_starts)
        for expected, observed in zip(starts, iset.interval_starts):
            assert observed == expected

        assert len(ends) == len(iset.interval_ends)
        for expected, observed in zip(ends, iset.interval_ends):
            assert observed == expected

    @hyp.given(intervals_strategy())
    def test_add_hypothesis(self, intervals):
        iset = IntervalSet()
        for interval in intervals:
            iset.add(interval)
            self.assert_consistency(iset)
        ...
        for permutation in itertools.permutations(intervals):
            other_iset = IntervalSet()
            for interval in permutation:
                other_iset.add(interval)
                self.assert_consistency(other_iset)
            assert iset.interval_starts == other_iset.interval_starts
            assert iset.interval_ends == other_iset.interval_ends
    ...
    def test_remove(self, intervals, starts, ends):
        iset = IntervalSet()
        iset.add(Interval(start=AxisBound.start(), end=AxisBound.end()))

        for interval in intervals:
            iset.remove(interval)
            self.assert_consistency(iset)
    ...
    @hyp.given(intervals_strategy())
    def test_remove_hypothesis(self, intervals):
        iset = IntervalSet()
        iset.add(Interval(start=AxisBound.from_start(-5), end=AxisBound.from_end(5)))
        ...
        for permutation in itertools.permutations(intervals):
            other_iset = IntervalSet()
            other_iset.add(Interval(start=AxisBound.from_start(-5), end=AxisBound.from_end(5)))
            for interval in permutation:
                other_iset.remove(interval)
                self.assert_consistency(other_iset)
            assert iset.interval_starts == other_iset.interval_starts
            assert iset.interval_ends == other_iset.interval_ends
